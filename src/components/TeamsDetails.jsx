...
const TeamsDetails = () => {
  ...
  return (
    ...
    <h4 className='mt-5 '>
      <Link className='px-4 py-2 rounded-md hover:bg-slate-800 bg-slate-900' to={{ pathname:'./players',search:`?teamId=${teamId}` }}>
        View Players
      </Link>
    </h4>

    <h4 className='mt-6 text-4xl text-center'>Championships</h4>
    ...
    <h2 className='py-2 text-2xl font-semibold'>Blog Articles</h2>
        <ul className='mx-auto '>
          {articles?.map((item)=>(
            <li className='my-4 transition rounded-md ' key={item.id}>
              <Link className='px-5 py-2 rounded-lg bg-slate-500 hover:bg-slate-400' to={`articles/${slugify(item.title)}`}>{item.title}</Link>
